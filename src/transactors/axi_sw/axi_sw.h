...
public:
  axi_sw(cvm::topology::loc_t loc, unsigned id)
      : loc_(loc), id_(id), name_(_axi_sw::string_to_lower(cvm::topology::name(loc_)) + std::to_string(id)),
  ...
  cvm::messenger::task<void> a(const axi::a_t& p) {
    co_await axi_->a(std::forward<const axi::a_t>(p));
    co_return;
  }

  cvm::messenger::task<void> w(axi::w_t&& p) {
    co_await axi_->w(std::forward<axi::w_t>(p));
    co_return;
  }
